...
def summarize_pdf(pdf_path):
    """Gemini 2.0 모델 적용 및 요약 로직"""
    if not GEMINI_API_KEY: return "Gemini 키 누락"
    print("🤖 Gemini 2.0 분석 중...")
    client = genai.Client(api_key=GEMINI_API_KEY)
    
    # 2026년 기준 가장 안정적인 2.0-flash 모델 사용
    # 모델명에서 'models/'를 빼고 시도합니다.
...
def upload_to_gdrive(file_path):
    ...
    # supportsAllDrives=True 옵션을 넣어 공유 드라이브 환경에서도 작동하게 함
...
if __name__ == "__main__":
    ...
    if pdf_file:
        ...
        try:
            ...
            print(f"❌ 드라이브 업로드 실패 원인: {e}")
            print("💡 팁: 개인 계정이라면 '공유 드라이브'를 사용해야 서비스 계정 업로드가 가능합니다.")
